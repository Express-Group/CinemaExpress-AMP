...
class Article_model extends CI_Model{
	...
	public function menuDetails(){
		$CacheID = 'menulist';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == ''){
			$menus = $this->live_db->query("SELECT Section_id , URLSectionStructure , Sectionname  FROM sectionmaster WHERE Status=1 AND ParentSectionID IS NULL AND IsSubSection=0 AND IsSeperateWebsite=0 AND MenuVisibility=1 ORDER BY DisplayOrder ASC LIMIT 12")->result_array();
	...
	public function galleryRelatedDetails($contentID){
		return $this->live_db->query("SELECT gallery_image_path , gallery_image_title , gallery_image_alt FROM gallery_related_images WHERE content_id='".$contentID."' ORDER BY display_order ASC")->result_array();
	}
}
?> 